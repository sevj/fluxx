...
#[AsCommand(name: 'fluxx:step:retry', description: 'Retry a workflow run from a specific step.')]
final class RetryWorkflowStepCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addArgument('step-code', InputArgument::REQUIRED, 'Restart step code.')
            ...
            ->addOption('force', null, InputOption::VALUE_NONE, 'Retry even if the original run is still in progress (use only when the original worker is definitely stuck).');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            $newRunId = $this->workflowRetryOperator->retryStep(
                runId: (string) $input->getArgument('run-id'),
                stepCode: (string) $input->getArgument('step-code'),
        ...
        $io->success(sprintf('Workflow step retried. New run ID: %s', $newRunId));
